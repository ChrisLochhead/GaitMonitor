...
def stitch_dataset(folder_names):
    file_name = '/Absolute_Data'
    datasets = load_whole_dataset(folder_names, file_name, override = True)
    ...
    for i, dataset in enumerate(datasets):
        ...
        if i > 0:
            current_instance, whole_dataset = Creator.assign_person_number(whole_dataset, dataset, 
                                                                       "./Code/Datasets/Joint_Data/WeightGait/" + str(i + 1) + "_people",
                                                                       i, current_instance, save)
    print("completed.")
